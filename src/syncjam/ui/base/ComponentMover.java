...
public class ComponentMover extends MouseAdapter
{
    ...
    public void mouseDragged(MouseEvent e)
    {
        ...
        Rectangle r = getBoundingRect( destination );

        while (locationX < r.getX() + edgeInsets.left)
            locationX += snapSize.width;

        while (locationY < r.getY() + edgeInsets.top)
            locationY += snapSize.height;

        while (locationX + destination.getSize().width  > r.getX()+r.getWidth() - edgeInsets.right)
            locationX -= snapSize.width;

        while (locationY + destination.getSize().height > r.getY()+r.getHeight() - edgeInsets.bottom)
            locationY -= snapSize.height;
    ...
    private Rectangle getBoundingRect(Component source)
    {
        Rectangle bounds;
        if (source instanceof Window)
        {
