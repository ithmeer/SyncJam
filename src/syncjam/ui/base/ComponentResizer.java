...
public class ComponentResizer extends MouseAdapter
{
    ...
    public ComponentResizer(Insets dragInsets, Dimension snapSize, Component... components)
    {
        ...
        sourceCursor = Cursor.getDefaultCursor();
    }
    ...
    protected void changeBounds(Component source, int direction, Rectangle bounds, Point pressed, Point current)
    {
        ...
        if (WEST == (direction & WEST))
        {
            ...
            Rectangle r = getBoundingRect( source );
            int maximum = Math.min(width + x - (int)r.getX(), maximumSize.width);
        ...
        if (NORTH == (direction & NORTH))
        {
            ...
            Rectangle r = getBoundingRect( source );
            int maximum = Math.min(height + y - (int)r.getY(), maximumSize.height);
        ...
        if (EAST == (direction & EAST))
        {
            ...
            Rectangle r = getBoundingRect( source );
            int maximum = Math.min((int)(r.getX()+r.getWidth()) - x, maximumSize.width);
            drag = getDragBounded(drag, snapSize.width, width, minimumSize.width, maximum);
            width += drag;
        }
        ...
        if (SOUTH == (direction & SOUTH))
        {
            ...
            Rectangle r = getBoundingRect( source );
            int maximum = Math.min((int)(r.getY()+r.getHeight()) - y, maximumSize.height);
            drag = getDragBounded(drag, snapSize.height, height, minimumSize.height, maximum);
            height += drag;
        }
    ...
    private Dimension getBoundingSize(Component source)
    {
        if (source instanceof Window)
        {
            ...
            Rectangle bounds = getAllScreenBounds(); //env.getMaximumWindowBounds();
            return new Dimension(bounds.width*2, bounds.height);
        }
        else
        {
    ...
    public Rectangle getBoundingRect(Component source)
    {
        Rectangle bounds;
        if (source instanceof Window)
        {
            //GraphicsEnvironment env = GraphicsEnvironment.getLocalGraphicsEnvironment();
            bounds = getAllScreenBounds(); //env.getMaximumWindowBounds();
        }
        else
        {
            int w = (int)source.getParent().getSize().getWidth();
            int h = (int)source.getParent().getSize().getHeight();
